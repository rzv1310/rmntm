  ...
  return (
    <div onMouseEnter={() => setActive(item)} className="relative ">
      <motion.p
        ...
        className="cursor-pointer text-foreground hover:text-primary font-light uppercase"
      >
        {item}
      </motion.p>
      {active !== null && (
...
export const Menu = ({
  ...
  return (
    <nav
      ...
      className="relative border border-border bg-background/95 backdrop-blur-md shadow-medium flex justify-center items-center space-x-8 px-8 py-4" style={{borderRadius: '10px'}}
    >
      {children}
    </nav>
  );
...
export const HoveredLink = ({ children, to, ...rest }: any) => {
  return (
    <Link
      to={to}
      {...rest}
      className="text-muted-foreground hover:text-foreground transition-colors py-2 px-3 rounded-lg font-light uppercase"
    >
      {children}
    </Link>
  );
